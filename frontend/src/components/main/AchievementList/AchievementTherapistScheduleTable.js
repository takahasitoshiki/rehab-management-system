...
import { getCompletedReservations, selectReservationsLoading, } from "@/store/slices/reservationSlice";
...
const AchievementTherapistScheduleTable = ({ dataSource, handleRowDoubleClick, onDropPatient, patients }) => {
    ...
    const completedReservations = useSelector((state) => state.reservations.completedReservations);
    useEffect(() => {
    ...
    const getTherapistSchedule = (therapistId, date) => {
        ...
        dataSource.forEach((slot) => {
            const matchingReservations = completedReservations.filter((res) => res.therapist_id.trim().toUpperCase() ===
